...
enum GAME_STATE {GAME_START, GAME_PROCESSING, GAME_END, GAME_RESUME, GAME_UNKNOWN};
...
void game_main()
{
	...
	PLANE_HELPER plane_helper = {POINT(100, 80), 1, 0, 0};
...
void game_start()
{
	...
	while(true)
	{
		if(redraw)
		{
			redraw = 0;
			UpdateScreen();
			draw_str((BK_W - 9 * times * 8) / 2, (BK_H - 2 * times * 8) / 2,
				0x3f, times, "plot mode");
			draw_str((BK_W - 11 * times * 8) / 2, (BK_H + 2 * times * 8) / 2,
		...
		switch(g_InputState.dir)
		{
			...
			case kdirRight:
				...
				if(sel & 0x1)
				{
					draw_str((BK_W - 9 * times * 8) / 2, (BK_H - 2 * times * 8) / 2, color[0], times, "plot mode");
				}
				else
				{
					draw_str((BK_W - 11 * times * 8) / 2, (BK_H + 2 * times * 8) / 2, color[0], times, "random mode");
				}
				break;
			default:break;
		}
		...
		if(g_InputState.dwKey == SDLK_RETURN)
		{
			if(sel & 0x1)
			{
				game_main();
...
void game_over()
{
	...
	while(jiffy ++ < 1500)
	{
		...
		draw_str((BK_W - 9 * 8 * 8) / 2, (BK_H - 8 * 8) / 2,
				0x3f, 8, "GAME OVER!");
