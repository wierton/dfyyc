...
void add_drop_xbullets(char ch)
{
	...
	for(i = 0; i < NR_XBULLETS_LEFT; i++)
	{
		...
		{
			drop_xbullets[ed_drop_xbullets].pos = POINT(rand() % BK_W, 1);
	...
	while(i != ed_drop_xbullets)
	{
		...
		if(!(drop_xbullets[i].dead))
		{
			if(drop_xbullets[i].pos.y < BK_H)
				draw_str(drop_xbullets[i].pos.x, drop_xbullets[i].pos.y, 0x3fa03f, 2, "%c", drop_xbullets[i].ch);
			else
			{
				op_drop_xbullets ++;
...
void draw_toolbar()
{
	uint32_t i, j = 3;
	const int dist = 25;
	draw_str(BK_W - 250, dist * 1, 0x3f, 2, "score:%d", score);
	draw_str(BK_W - 250, dist * 2, 0x3f, 2, "life:%d/%d", query_player_life(), query_life_atk(ID_PLAYER));
	...
	for(i = 0; i < NR_XBULLETS_LEFT; i++)
	{
		...
		if(xbullets[dir] > 0)
		{
			draw_str(BK_W - 250, dist * (j ++), 0x3f, 2, "%c:%d", xbullets_left[i], xbullets[dir]);
		}
	}
}
